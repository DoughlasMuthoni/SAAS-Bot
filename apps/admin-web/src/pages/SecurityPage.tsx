...
export default function SecurityPage() {
  const { data, isLoading, isError } = useQuery<Page>({
    queryKey: ['public-page', 'security'],
    queryFn: () => fetch('/api/v1/public/pages/security').then(r => r.ok ? r.json() : Promise.reject(r.status)),
    staleTime: 60_000,
  })
  ...
  if (isError || !data) return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 0 80px' }}>
      <p style={{ color: '#dc2626' }}>Unable to load Security page. Please try again later.</p>
    </div>
  )
