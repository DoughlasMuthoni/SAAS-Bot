...
export default function TermsOfServicePage() {
  const { data, isLoading, isError } = useQuery<Page>({
    ...
    queryFn: () => fetch('/api/v1/public/pages/terms').then(r => r.ok ? r.json() : Promise.reject(r.status)),
    staleTime: 60_000,
  })
  ...
  if (isError || !data) return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 0 80px' }}>
      <p style={{ color: '#dc2626' }}>Unable to load Terms of Service. Please try again later.</p>
    </div>
  )
