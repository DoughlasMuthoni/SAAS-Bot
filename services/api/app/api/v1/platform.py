...
async def record_payment(
    ...
    try:
        inv = await PlatformService.record_payment(db, invoice_id, body)
    except ValueError as e:
        raise HTTPException(status_code=422, detail=str(e))
    if inv is None:
        raise HTTPException(status_code=404, detail="Invoice not found")
...
<style>
  *{{box-sizing:border-box;margin:0;padding:0}}
  ...
  .footer-logo img{{height:24px;width:auto;opacity:.5}}
  @media print{{
    body{{padding:20px}}
...
<table class="totals" style="width:280px;margin-left:auto">
  ...
  {f'<tr><td style="color:#15803d;font-weight:600;padding-top:6px">Amount Paid</td><td style="text-align:right;color:#15803d;font-weight:600;padding-top:6px">{inv.currency} {float(inv.amount_paid):,.2f}</td></tr><tr><td style="color:#dc2626;font-weight:700;padding-top:4px">Balance Due</td><td style="text-align:right;color:#dc2626;font-weight:700;padding-top:4px">{inv.currency} {float(inv.balance_due):,.2f}</td></tr>' if inv.status == "partial" else ""}
  {f'<tr><td style="color:#15803d;font-weight:600;padding-top:6px">Paid in Full</td><td style="text-align:right;color:#15803d;font-weight:600;padding-top:6px">{inv.paid_at[:10] if inv.paid_at else ""}</td></tr>' if inv.status == "paid" else ""}
</table>

{f'<div class="notes"><strong style="display:block;margin-bottom:4px;font-size:11px;text-transform:uppercase;letter-spacing:.05em;color:#0f172a">Notes / Payment Instructions</strong>{inv.notes}</div>' if inv.notes else ''}
